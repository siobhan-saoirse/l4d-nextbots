...
local function getAllInfected()
	...
	if (math.random(1,16) == 1) then
		for k,v in ipairs(ents.GetAll()) do
			if (v:GetClass() == "npc_spitter") then
				if (v:Health() > 1) then
					table.insert(npcs, v)
				end
			end
	...
	if (math.random(1,16) == 1) then
		for k,v in ipairs(ents.FindInSphere( ply:GetPos(), 120000 )) do
			...
			if (engine.ActiveGamemode() == "teamfortress") then
				...
				if ((v:IsPlayer() && !GetConVar("ai_ignoreplayers"):GetBool() || v:IsNPC() || v:IsNextBot() && string.find(v:GetClass(),"survivor")) and v:GetClass() != "npc_spitter"  and v:GetClass() != "infected" and v:EntIndex() != ply:EntIndex()) then
					if (v:Health() > 1) then
						table.insert(npcs, v)
					end
				end
...
hook.Add("EntityEmitSound","spitterHearSound",function(snd)
	if (IsValid(snd.Entity)) then 
		if IsValid(snd.Entity) and snd.Entity:GetModel() and string.StartWith(snd.Entity:GetModel(), "models/infected/spitter") and string.find(snd.SoundName, "step") then
	...
	if (IsMounted("left4dead") or IsMounted("left4dead2")) then 
			...
			return true
		elseif IsValid(snd.Entity) and snd.Entity:GetModel() and string.find(snd.SoundName,"female") and string.find(snd.Entity:GetModel(),"spitter") then
		...
		elseif ((snd.Entity:IsPlayer() && !GetConVar("ai_ignoreplayers"):GetBool()  || snd.Entity:IsNPC()) and !snd.Entity:IsNextBot() and snd.Entity:GetClass() != "infected") then
			for k,v in ipairs(ents.FindInSphere(snd.Entity:GetPos(),6000)) do
				if (v:GetClass() == "npc_spitter" and !IsValid(v:GetEnemy()) and v.Ready and !v.ContinueRunning and !v:IsOnFire() and !snd.Entity:IsFlagSet(FL_NOTARGET) and snd.Entity:Visible(v)) then
...
function ENT:Shove(anim)
	self:EmitSound("spitterZombie.Shoved")
end
...
function ENT:Initialize()
	...
	if SERVER then
		...
		self:EmitSound("spitterZombie.Gurgle")
...
function ENT:OnRemove()
	if SERVER then
		self:StopSound("spitterZombie.Gurgle")
	end
		...
		for k,v in ipairs( _ents ) do
			if (engine.ActiveGamemode() == "teamfortress") then
				if ( ( v:IsPlayer() or v:IsNPC()) and !v:IsFriendly(self) and GAMEMODE:EntityTeam(v) != TEAM_SPECTATOR and GAMEMODE:EntityTeam(v) != TEAM_FRIENDLY and v:Health() > 1 and !v:IsFlagSet(FL_NOTARGET) ) then
					...
					self:SetEnemy(v)
					--self:EmitSound("spitterZombie.RageAtVictim")
				...
				if ( ( v:IsPlayer() or v:IsNPC()) and !v:IsNextBot() and v:GetClass() != "npc_spitter"  and v:GetClass() != "infected" and v:Health() > 0 and !v:IsFlagSet(FL_NOTARGET) ) then
					-- We found one so lets set it as our enemy and return true
					self:SetEnemy(v)
					--self:EmitSound("spitterZombie.RageAtVictim")
					if (v:IsNPC() and v:Classify() != CLASS_ZOMBIE) then
...
hook.Add("ScaleNPCDamage","spitterDamage",function(npc,hitgroup,dmginfo)
	
	if (npc:GetClass() == "npc_spitter") then	
	...
	if SERVER then 
		...
		if (!self:IsOnGround()) then
			if (!self.HaventLandedYet) then 
				...
				self:EmitSound("spitterZombie.Fall")
				self.FallDamage = 10;
				timer.Create("BurpWhileFalling"..self:EntIndex(), 0.8, 0, function()
					if (!self:IsOnGround()) then

						self:EmitSound("spitterZombie.Pounce.FlyLoop")
			...
			if (self.HaventLandedYet) then
				...
				if (self.Pouncing) then
					self:EmitSound("spitterZombie.Pounce.Miss")
				else
			...
			if (IsValid(self.Door) and !self.ContinueRunning and self.Door:GetPos():Distance(self:GetPos()) < self.AttackRange) then
				...
				if (!self.MeleeAttackDelay2 or CurTime() > self.MeleeAttackDelay2) then
					if (self.Door:GetPos():Distance(self:GetPos()) < self.AttackRange) then
						...
						self:SetEnemy(nil)
						local selanim = self:LookupSequence("Spitter_Melee_0"..math.random(1,2))
	...
	while ( path:IsValid() and IsValid(self:GetEnemy()) and !self.ContinueRunning and !self.PlayingSequence and !self.PlayingSequence3 and !self.PlayingSequence2 ) do
		...
		if (self:GetEnemy():GetPos():Distance(self:GetPos()) > self.AttackRange) then
			for k,v in ipairs(ents.FindInSphere(self:GetPos(),180)) do -- avoid other infected
				if (v:GetClass() == "npc_spitter" and v:EntIndex() != self:EntIndex()) then
		...
		for k,v in ipairs(ents.FindInSphere(self:GetPos(),120)) do
			if (v ~= self and IsValid(v) and (v.IsInfected or v:GetClass() == "npc_spitter")) then
				self:SetCollisionGroup(COLLISION_GROUP_DEBRIS)
			else
				self:SetCollisionGroup(COLLISION_GROUP_NPC)
			end
...
function ENT:OnInjured( dmginfo )
	...
	if (self:Health() > 0 and (!self.PainSoundTime or CurTime() > self.PainSoundTime)) then
		if (dmginfo:IsDamageType(DMG_BURN)) then
			self:EmitSound("spitterZombie.Pain")
		else
			self:EmitSound("spitterZombie.PainShort")
		end
	...
	if (dmginfo:IsDamageType(DMG_BULLET) and self:Health() > 0) then
		if (math.random(1,6) == 1) then
			self:EmitSound("spitterZombie.BulletImpact")
		end
	end
	if (!dmginfo:IsDamageType(DMG_BURN)) then
	...
	if SERVER then
		...
		self.Ready = false
				if (IsValid(self)) then
					...
					self:EmitSound("spitterZombie.Death")
