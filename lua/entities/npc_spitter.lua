	...
	if (math.random(1,16) == 1) then
		for k,v in ipairs(ents.FindInSphere( ply:GetPos(), 120000 )) do
			...
			if (engine.ActiveGamemode() == "teamfortress") then
				...
				if ((v:IsPlayer() && !GetConVar("ai_ignoreplayers"):GetBool() || v:IsNPC()) and !v:IsNextBot() and v:GetClass() != "npc_spitter"  and v:GetClass() != "infected" and v:EntIndex() != ply:EntIndex()) then
					if (v:Health() > 1) then
						table.insert(npcs, v)
					end
				end
